...
def build_noisy_sampler(tparams, options, trng):
    ...
    injd_noise = tensor.matrix('injected_noise', dtype='float32')
    ...
    # inject noise
    init_state = prev_state + injd_noise  # apply the injected noise
    ...
    # compile the function read-out and samples
    print 'Building f_sim_next..',

    inps      = [y, ctx, mask, prev_state, injd_noise]
    ctxdim    = options['dim'] if not options.get('birnn', False) else 2 * options['dim']
    readout   = tensor.concatenate([next_state, ctxs, emb], axis=-1)  # the obersavtion for each step.
    options['readout_dim'] = options['dim_word'] + options['dim'] + ctxdim

    outs = [next_probs, next_sample, next_state, readout, attention]
    f_sim_next = theano.function(inps, outs, name='f_sim_next', profile=profile)
